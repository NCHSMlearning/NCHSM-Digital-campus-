...
function showLoading(msg) {
    ...
    l.innerHTML = `
        ...
        <div class="loader-text">${msg || 'Loading...'}</div>
    `;
...
// Show notification
function showNotification(msg, isErr = false, type = '') {
...
async function getMarkEntrySettings() {
    try {
        ...
        console.error('Error in getMarkEntrySettings:', error);
        return { global: { enabled: true } };
    }
}

// Log mark entry action
...
async function apiCall(endpoint, opts = {}) {
    try {
        ...
        if (endpoint.match(/^\/api\/marks\/.+\/.+$/) && !opts.method) {
            ...
            console.log(`📡 Getting marks: ${block}/${subject}, Year: ${year}, Type: ${examType}`);
            ...
            if (examType === 'nck') {
                ...
                if (error) {
                    console.error('Error fetching NCK marks:', error);
                    return [];
                }
        ...
        if (endpoint === '/api/marks' && opts.method === 'POST') {
            ...
            console.log(`📡 Saving ${marksData.length} marks: ${block}/${subject}`);
            ...
            if (examType === 'nck') {
                ...
                for (const mark of marksData) {
                    try {
                        ...
                        console.error('Error saving NCK mark:', markError);
                ...
                for (const mark of marksData) {
                    try {
                        ...
                        console.error('Error saving mark:', markError);
                        errors.push({ admission: mark.admission, error: markError.message });
                    }
                }
            }
        ...
        if (endpoint === '/api/mark-entry/toggle-global' && opts.method === 'POST') {
            ...
            return {
                success: true,
                message: newEnabled ? 'Global mark entry opened' : 'Global mark entry closed',
                enabled: newEnabled
            };
        }
        ...
        if (endpoint === '/api/mark-entry/toggle-class' && opts.method === 'POST') {
            ...
            return {
                success: true,
                message: newEnabled ? `${year} class entry opened` : `${year} class entry closed`,
                enabled: newEnabled
            };
        }
        ...
        if (endpoint === '/api/mark-entry/toggle-subject' && opts.method === 'POST') {
            ...
            return {
                success: true,
                message: newEnabled ? `Entry opened for ${subject}` : `Entry closed for ${subject}`,
                enabled: newEnabled
            };
        }
        
        // ===== FALLBACK: Return error =====
        return { success: false, error: 'Endpoint not implemented in Supabase' };
...
async function refreshAllData() {
    ...
    hideLoading();
    showNotification('✅ Data refreshed from Supabase!', false);
...
function renderWithSidebar(contentHtml) {
    ...
    // Year/Exam type changes (admin only)
    if (isAdmin) {
...
async function showMain() {
    ...
    const statsHtml = `
        <div class="selector-group">
            <div>
                <label><i class="fas fa-calendar-alt"></i> Class Year:</label> 
                <select id="yearSelectMain" onchange="changeYear()">
                    ${INTAKE_YEARS.map(y => `<option value="${y}" ${y===currentYear?'selected':''}>🎓 March ${y} Class</option>`).join('')}
                </select>
            </div>
            <div>
                <label><i class="fas fa-file-alt"></i> Exam Type:</label> 
                <select id="examTypeSelect" onchange="changeExamType()">
                    <option value="internal" ${currentExamType==='internal'?'selected':''}>📝 Internal Exams (CAT1+CAT2+Exam)</option>
                    <option value="nck" ${currentExamType==='nck'?'selected':''}>🏥 NCK Score Sheet (Clinical Assessment)</option>
                </select>
            </div>
        </div>
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-label"><i class="fas fa-layer-group"></i> Academic Blocks</div>
            </div>
            <div class="stat-card">
                <div class="stat-number">${isAdmin ? (stats.totalSubjects || 0) : (currentUser?.subjects?.length || 0)}</div>
                <div class="stat-label"><i class="fas fa-book"></i> Active Units</div>
            </div>
            <div class="stat-card">
                <div class="stat-number">60%</div>
                <div class="stat-label"><i class="fas fa-flag-checkered"></i> Pass Mark</div>
            </div>
        </div>
    `;
    ...
    // Year/Exam type change (dashboard version)
...
async function showAdminMarks() {
    const html = `
        ...
        <div class="header">
            <h3><i class="fas fa-pen-alt"></i> Internal Marks Entry</h3>
            <p>Select a block and subject to enter marks</p>
        </div>
        <select id="blockSelect" class="full-width">
            <option value="">-- Select Block --</option>
            ${['BLOCK_0','BLOCK_1','BLOCK_2','BLOCK_3','BLOCK_4','BLOCK_5'].map(b => 
                `<option value="${b}">${b.replace('_', ' ')}</option>`
            ).join('')}
        </select>
        <select id="subjectSelect" class="full-width" style="display:none;">
            <option value="">-- Select Subject --</option>
        </select>
        <div id="marksTableContainer">
            <div class="card text-center">
                <p>Select a block and subject to view marks</p>
            </div>
        </div>
    `;
...
async function loadAdminMarks(block, subject, at) {
    ...
    let html = `
        <div style="margin-bottom:16px; display:flex; gap:12px; flex-wrap:wrap;">
            ...
            <button class="btn-info" onclick="fillDownValues()" style="padding:8px 20px;border:none;border-radius:40px;color:white;background:linear-gradient(135deg,#3b82f6,#2563eb);cursor:pointer;">
    ...
    for (let i = 0; i < marks.length; i++) {
        ...
        if (at === 'full') {
            html += `
                <td style="padding:10px;"><input type="number" id="acat1_${i}" value="${cat1}" min="0" max="30" step="0.5" onchange="updateAdminTotal(${i})"></td>
            ...
            html += `
                <td style="padding:10px;"><input type="number" id="aexam_${i}" value="${exam}" min="0" max="100" step="0.5" onchange="updateAdminTotal(${i})"></td>
            `;
        }
        
        html += `
                <td id="atotal_${i}" style="font-weight:bold;">${total}</td>
    ...
    html += `
        ...
        <div style="text-align:center;margin-top:20px;">
            <button class="save-btn" onclick="saveAdminMarks()">💾 Save All Marks</button>
        </div>
    `;
...
window.saveAdminMarks = async function() {
    ...
    const result = await apiCall('/api/marks', {
        ...
        body: JSON.stringify({
            ...
            marksData: marksData,
            lecturerName: currentUser?.name || 'System'
        })
    });
    hideLoading();
...
function fillDownValues() {
    const inputs = document.querySelectorAll('#marksTableContainer input[type="number"], #nck-marks-container input[type="number"]');
    if (inputs.length === 0) {
...
async function showNCKMarks() {
    const html = `
        <button class="back-btn" onclick="switchTab('dashboard')"><i class="fas fa-arrow-left"></i> Back</button>
        <div class="header">
            <h3><i class="fas fa-file-medical"></i> NCK Score Sheet Entry</h3>
            <p>Clinical evaluation (XY FORMS) & Assessment & Case</p>
        </div>
        <div class="selector-group">
        ...
        <div id="nckMarksContainer">
            <div class="card text-center">
                <p>Select a sheet and click "Load Data"</p>
            </div>
        </div>
    `;
...
function displayXYFormsMarks(marks) {
    ...
    let html = `
        <div style="margin-bottom:15px;padding:12px;background:linear-gradient(135deg,#667eea20,#764ba220);border-radius:12px;">
            <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
                ...
                <button class="save-btn" onclick="openFastEntryPanel()" style="padding:5px 12px;background:#8b5cf6;border:none;border-radius:40px;color:white;cursor:pointer;">
    ...
    for (let i = 0; i < marks.length; i++) {
        ...
        html += `<tr class="${rowClass}">
            ...
            <td style="position:sticky;left:40px;background:white;font-weight:500;cursor:pointer;color:#6366f1;text-decoration:underline;" 
        ...
        html += `
                    <td id="xyAvg_${i}" style="font-weight:bold;text-align:center;background:${avg>=60?'#d1fae5':'#fee2e2'};">${avg.toFixed(2)}</td>
    ...
    if (sheetName === 'XY FORMS') {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            marksData.push({
                row: i + 2,
                scores: scores,
                gradedBy: document.getElementById(`xyGraded_${i}`)?.value || ''
            });
        }
    } else {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            marksData.push({
                row: i + 2,
                scores: scores,
                gradedBy: document.getElementById(`acGraded_${i}`)?.value || ''
            });
        }
    }
...
async function showStudents() {
    ...
    for (const s of students) {
        html += `
            <tr class="${s.status === 'ACTIVE' ? 'pass-row' : 'fail-row'}">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="field">
                        ...
                        <input type="text" id="studentAdmission" placeholder="e.g., KRCHN/0001/26">
                    </div>
                    <div class="field">
...
async function saveNewStudent() {
    ...
    // This would need a Supabase insert - for now using proxy
    // Since we don't have a direct student insert endpoint in Supabase yet
    const result = await apiCall('/api/add-student', {
        method: 'POST',
        body: JSON.stringify({ admission, name, block })
    });
    hideLoading();
    closeModal();
    ...
    if (result?.success) {
        ...
        showNotification(result?.message || 'Error adding student', true);
    }
}
...
async function showLecturers() {
    ...
    let html = `
        <button class="back-btn" onclick="switchTab('dashboard')"><i class="fas fa-arrow-left"></i> Back</button>
        <div class="header">
            <h3><i class="fas fa-chalkboard-user"></i> Manage Lecturers</h3>
            <div class="header-actions">
                <button class="save-btn" onclick="showAddLecturerModal()"><i class="fas fa-plus"></i> Add Lecturer</button>
            </div>
        </div>
    `;
    ...
    for (const l of lecturers) {
        ...
        html += `
            <div class="lecturer-card">
                <div class="info">
                    <strong>${l.full_name || l.name}</strong>
                    <br><small>${l.email || l.username}</small>
                </div>
...
async function showUnits() {
    ...
    let html = `
        <button class="back-btn" onclick="switchTab('dashboard')"><i class="fas fa-arrow-left"></i> Back</button>
        <div class="header">
            <h3><i class="fas fa-book-open"></i> Manage Units</h3>
        </div>
        <div class="card">
            ...
            <div class="form-grid">
                ...
                <div>
                    ...
                    <input type="text" id="newUnitName" placeholder="Subject Name">
                </div>
                <div>
                ...
                <div>
                    <button class="save-btn" onclick="addNewUnit()"><i class="fas fa-plus"></i> Add Unit</button>
                </div>
            </div>
        </div>
    `;
        ...
        if (blockUnits.length) {
            html += `
                <div style="margin-top:24px;">
                    <h3 style="display:inline-block;background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:8px 24px;border-radius:40px;">${block.replace('_', ' ')}</h3>
...
async function showEntryControlPanel() {
    ...
    let html = `
        <button class="back-btn" onclick="switchTab('dashboard')"><i class="fas fa-arrow-left"></i> Back</button>
        <div class="header">
            <h3><i class="fas fa-lock"></i> Mark Entry Control Panel</h3>
            <p>Control mark entry access for lecturers</p>
        </div>
        <div class="card">
            ...
            <div class="setting-card">
                <div class="setting-row">
                    <div>
                        <strong>🔓 All Mark Entry</strong>
                        <br><small>Master control for all mark entry</small>
                    </div>
            ...
            ${['BLOCK_0','BLOCK_1','BLOCK_2','BLOCK_3','BLOCK_4','BLOCK_5'].map(block => `
                <div class="setting-card">
                    <div class="setting-row">
                        ...
                        <button class="btn-info" onclick="showBlockSubjectControl('${block}')" style="padding:8px 16px;border:none;border-radius:40px;color:white;cursor:pointer;">
        ...
        <div class="card">
            ...
            <div style="max-height:400px;overflow-y:auto;">
                ${logs.length === 0 ? '<p style="text-align:center;padding:20px;">No logs yet</p>' : 
...
async function showComprehensiveAnalytics() {
    ...
    blocks.forEach(b => blockData[b] = { scores: [], count: 0 });
    
    marks?.forEach(m => {
        if (blockData[m.block]) {
            blockData[m.block].scores.push(m.final_score || 0);
            blockData[m.block].count++;
        }
    });
    ...
    blocks.forEach(b => {
        ...
        blockAverages[b] = data.scores.length ? (data.scores.reduce((a, c) => a + c, 0) / data.scores.length).toFixed(1) : 0;
    });
    
    // Grade distribution
    const gradeDist = {
        'A (80-100%)': marks?.filter(m => (m.final_score || 0) >= 80).length || 0,
        'B (70-79%)': marks?.filter(m => (m.final_score || 0) >= 70 && (m.final_score || 0) < 80).length || 0,
        'C (60-69%)': marks?.filter(m => (m.final_score || 0) >= 60 && (m.final_score || 0) < 70).length || 0,
        'D (50-59%)': marks?.filter(m => (m.final_score || 0) >= 50 && (m.final_score || 0) < 60).length || 0,
        'F (<50%)': marks?.filter(m => (m.final_score || 0) < 50 && (m.final_score || 0) > 0).length || 0
    };
    ...
    const html = `
        ...
        <div class="header">
            ...
            <p>March ${year} Class | Real-time Performance Intelligence</p>
        </div>
        
        <div class="analytics-grid">
            <div class="analytics-card">
                <h4><i class="fas fa-users"></i> Total Enrollment</h4>
                <div class="value">${totalStudents}</div>
                <small>Active Students</small>
            </div>
            <div class="analytics-card">
                <h4><i class="fas fa-chalkboard"></i> Subjects Offered</h4>
                <div class="value">${marks ? [...new Set(marks.map(m => m.subject_name))].length : 0}</div>
                <small>Across 6 Academic Blocks</small>
            </div>
            <div class="analytics-card">
                <h4><i class="fas fa-flag-checkered"></i> Overall Pass Rate</h4>
                <div class="value">${passRate}%</div>
                <small>${passed} passing / ${totalMarks} assessed</small>
            </div>
            <div class="analytics-card">
                <h4><i class="fas fa-chart-simple"></i> Class Average</h4>
                <div class="value">${avgScore.toFixed(1)}%</div>
                <small>Mean performance score</small>
            </div>
            <div class="analytics-card">
                <h4><i class="fas fa-hospital-user"></i> Clinical Average</h4>
                <div class="value">--</div>
                <small>NCK XY Forms Assessment</small>
            </div>
            <div class="analytics-card">
                <h4><i class="fas fa-trophy"></i> Top Student Score</h4>
                <div class="value">${marks?.length ? Math.max(...marks.map(m => m.final_score || 0)).toFixed(1) : 0}%</div>
                <small>${marks?.length ? marks.find(m => (m.final_score || 0) === Math.max(...marks.map(x => x.final_score || 0)))?.student_name || 'N/A' : 'N/A'}</small>
            </div>
        </div>
        ...
        <div class="analytics-grid">
            ...
            <div class="analytics-card">
                <h4><i class="fas fa-chart-line"></i> Block Performance</h4>
                <canvas id="blockChart" height="200"></canvas>
            </div>
        </div>
        
        <div class="card">
            <div class="card-title"><i class="fas fa-trophy"></i> 🏆 Honor Roll - Top 10 Students</div>
            <div style="overflow-x:auto;">
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ${marks?.length ? 
                            [...new Set(marks.map(m => m.admission_number))].slice(0, 10).map((adm, idx) => {
                                ...
                                return `
                                    <tr>
                                        ...
                                        <td><strong>${studentMarks[0]?.student_name || 'Unknown'}</strong></td>
                                        <td>${adm}</td>
                                        <td>${studentMarks.length}</td>
                                        <td><span class="stat-badge ${avg >= 80 ? 'high' : avg >= 60 ? 'medium' : 'low'}">${avg.toFixed(1)}%</span></td>
                                        <td><span class="badge ${avg >= 60 ? 'badge-pass' : 'badge-fail'}">${avg >= 60 ? 'PASS' : 'FAIL'}</span></td>
                                    </tr>
                                `;
                            }).join('') : 
    ...
    setTimeout(() => {
        ...
        if (perfCtx) {
            new Chart(perfCtx, {
                type: 'doughnut',
                data: {
                    labels: [`Passing (${passed})`, `Failing (${totalMarks - passed})`, `Pending (${totalStudents - totalMarks})`],
                    datasets: [{
                        data: [passed, totalMarks - passed, Math.max(0, totalStudents - totalMarks)],
        ...
        if (blockCtx) {
            new Chart(blockCtx, {
                type: 'bar',
                data: {
                    labels: blocks.map(b => b.replace('_', ' ')),
                    datasets: [{
                        label: 'Average Score (%)',
                        data: blocks.map(b => blockAverages[b] || 0),
...
async function showScorePublishPanel() {
    ...
    // Get published scores status from student_marks table
    const year = currentYear || '2026';
    const { data: marks } = await sb
        .from('student_marks')
        .select('*')
    ...
    marks?.forEach(m => {
        ...
        if (!subjects[key]) {
            subjects[key] = { marks: [], block: m.block, assessmentType: m.assessment_type };
        }
    ...
    const html = `
        <button class="back-btn" onclick="switchTab('dashboard')"><i class="fas fa-arrow-left"></i> Back</button>
        <div class="header">
            <h3><i class="fas fa-eye-slash"></i> Score Publishing Control Panel</h3>
            <p>Control which exam results students can see - Publish only when ready</p>
        </div>
        
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-number">${totalScores}</div>
                <div class="stat-label">Total Scores Entered</div>
            </div>
            <div class="stat-card" style="background: #d1fae5;">
                <div class="stat-number" style="color: #065f46;">${published}</div>
                <div class="stat-label">Published to Students</div>
            </div>
            <div class="stat-card" style="background: #fee2e2;">
                <div class="stat-number" style="color: #991b1b;">${hidden}</div>
                <div class="stat-label">Hidden from Students</div>
            </div>
            <div class="stat-card">
                <div class="stat-number">${publishRate}%</div>
        ...
        <div class="card">
            <div class="card-title">
                <i class="fas fa-chart-line"></i> Exam Scores - Manage Publication
                <button class="save-btn" onclick="showScorePublishPanel()" style="margin-left:auto;padding:6px 16px;">
                ...
                <table>
                    ...
                    <tbody>
                        ${Object.entries(subjects).map(([name, data]) => {
                            ...
                            const studentIds = data.marks.map(m => m.admission_number);
                            
                            return `
                                <tr>
                                    <td><strong>${name}</strong><br><small>${data.assessmentType === 'full' ? 'CAT1+CAT2+Exam' : (data.assessmentType === 'single_cat' ? 'CAT+Exam' : 'Exam Only')}</small></td>
                                    <td>${data.block?.replace('_', ' ') || 'N/A'}</td>
                                    <td style="text-align:center;">${total}</td>
                                    <td>
                                        <strong>${publishedCount}</strong> / ${total}
                                        <div class="progress-bar" style="margin-top:5px;width:80px;">
                                            <div class="progress-fill" style="width:${(publishedCount/total)*100}%;background:${publishedCount === total ? '#10b981' : '#f59e0b'};"></div>
                                        </div>
                                    </td>
                                    <td>${statusBadge}</td>
                                    <td>
                                        <button class="save-btn" onclick="publishSubject('${name}')" style="padding:4px 12px;font-size:11px;margin-right:4px;">
                                            <i class="fas fa-eye"></i> Manage
                                        </button>
                                        <button class="${isFullyPublished ? 'btn-warning' : 'btn-success'}" onclick="bulkPublishSubject('${name}')" style="padding:4px 12px;font-size:11px;border:none;border-radius:40px;color:white;cursor:pointer;">
                        ...
                        ${Object.keys(subjects).length === 0 ? '<tr><td colspan="6" style="text-align:center;">No marks data available</td></tr>' : ''}
                    </tbody>
                </table>
            </div>
        </div>
...
console.log('🚀 Nursing Marks System v2.0 loaded!');
