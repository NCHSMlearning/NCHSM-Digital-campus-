...
const LecturerAttendance = {
    ...
    async loadTodayAttendance() {
        ...
        try {
            ...
            // Do not fall back to the global geo_attendance_logs table here.
            // The lecturer screen is session-first: without a selected scheduled
            // session there is no valid class register to display.
            this.todayLogs = [];
            this.filteredTodayLogs = [];
            this.stats = { total: 0, present: 0, absent: 0, pending: 0, rate: 0 };
            tbody.innerHTML = `<tr><td colspan="10" style="padding:40px;text-align:center;color:#64748b;">
                <i class="fas fa-calendar-check" style="font-size:32px;display:block;margin-bottom:10px;color:#cbd5e1;"></i>
                <p style="margin:0;font-weight:600;">Select a scheduled attendance session</p>
                <small style="display:block;margin-top:6px;color:#94a3b8;">Each class occurrence has its own attendance register.</small>
            </td></tr>`;
            this.updateSessionStats({ total: 0, present: 0, absent: 0, pending: 0, rate: 0 });
        } catch (error) {
    ...
    async loadAttendanceStats() {
        try {
            ...
            // Never calculate the lecturer's class statistics from the global
            // attendance log. Without a selected session there is no class roster.
            this.updateSessionStats({ total: 0, present: 0, absent: 0, pending: 0, rate: 0 });
        } catch (error) {
    ...
    async markStudentAttendance(e) {
        ...
        try {
            ...
            if (!this.selectedSessionId) throw new Error('Select an attendance session before marking a student.');
            ...
            // Duplicate prevention is strictly scoped to the selected session.
            const { data: sessionLogs, error: duplicateError } = await supabase
                .from('geo_attendance_logs')
                .select('id, user_id, student_id, registration_number, attendance_status')
                .eq('session_id', this.selectedSessionId)
                .neq('role', 'lecturer');
            if (duplicateError) throw new Error(duplicateError.message);

            const studentKeys = [studentId, student.student_id, student.registration_number].filter(Boolean).map(String);
            const duplicate = (sessionLogs || []).find(log =>
                [log.user_id, log.student_id, log.registration_number].filter(Boolean).some(v => studentKeys.includes(String(v)))
            );
            if (duplicate) throw new Error(`${student.full_name || 'Student'} already has an attendance record for this session.`);

            const selectedSession = this.selectedSession || await this.getScheduledSessionById(this.selectedSessionId);
            if (!selectedSession) throw new Error('Selected attendance session could not be found.');
            ...
            const { error: insertError } = await supabase.from('geo_attendance_logs').insert({
                ...
                target_name: unit || selectedSession.session_title || selectedSession.title || 'General', unit_name: unit || selectedSession.unit_name || 'General',
    ...
    async exportCSV() {
        ...
        let source;
        let selectedExportSession = null;
        if (this.selectedSessionId) {
            selectedExportSession = this.selectedSession || await this.getScheduledSessionById(this.selectedSessionId);
            if (selectedExportSession) {
                const register = await this.getSessionAttendanceRegister(selectedExportSession, false);
                const sessionDate = selectedExportSession.session_date ? new Date(selectedExportSession.session_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0];
                source = (register.rows || []).map(row => ({
                    ...this.sessionRowToLog(row, selectedExportSession),
                    export_session_date: sessionDate
                }));
            }
        }
        if (!source) source = [...(this.todayLogs || [])].filter(l => l.session_type !== 'Lecturer Check-in');
        // A selected session is authoritative; do not discard absent/pending roster rows
        // merely because they do not have a check-in timestamp.
        if (filterDate && !selectedExportSession) source = source.filter(l => l.check_in_time && new Date(l.check_in_time).toISOString().split('T')[0] === filterDate);
        ...
        for (const cls of classList) {
            ...
            cls.logs.forEach(log => {
                const exportDate = log.export_session_date || (log.check_in_time ? new Date(log.check_in_time).toISOString().split('T')[0] : null);
                if (!exportDate) return;
                dateSet.add(exportDate);
            });
            ...
            cls.logs.forEach(log => {
                ...
                const iso = log.export_session_date || (log.check_in_time ? new Date(log.check_in_time).toISOString().split('T')[0] : null);
    ...
    async getSessionAttendanceRegister(session, finalize = false) {
        ...
        // One effective log per student. Index every identifier on the log so a
        // user_id/student_id/registration_number mismatch cannot create a duplicate
        // or make a real check-in disappear from the roster.
        const byStudent = new Map();
        const putLogForKey = (key, log) => {
            if (!key) return;
            const k = String(key).trim();
            if (!k) return;
            const previous = byStudent.get(k);
            if (!previous) { byStudent.set(k, log); return; }
            ...
            if ((currentIsPresent && !previousIsPresent) ||
                (!currentIsPresent && !previousIsPresent && new Date(log.check_in_time || 0) > new Date(previous.check_in_time || 0))) {
                byStudent.set(k, log);
            }
        };
        (logs || []).forEach(log => {
            [log.user_id, log.student_id, log.registration_number].filter(Boolean).forEach(key => putLogForKey(key, log));
        });
        ...
        for (const student of roster) {
            ...
            const status = String(log?.attendance_status || '').toLowerCase();
            const isManual = log?.is_manual_entry === true;
            const radius = Number(log?.target_radius ?? session.target_radius ?? 150);
            const distance = Number(log?.distance_meters);
            const hasDistance = Number.isFinite(distance);
            const inRadius = isManual ? true : (hasDistance && distance <= radius);
            const validPresent =
                !!log &&
