window.LecturerOnlineLearning = (() => {
    ...
    async function loadSubmissions(){
        ...
        const assignmentMap=new Map(state.assignments.map(a=>[String(a.id),a]));
        body.innerHTML=state.submissions.map(s=>{
            const p=map.get(s.student_id)||{};
            const a=assignmentMap.get(String(s.assignment_id))||{};
            const maxMarks=Number(s.max_marks||a.max_marks||a.online_marking_keys?.max_marks||0);
...
<div style="background:#fef9e7;border-left:4px solid #f39c12;border-radius:10px;padding:13px 16px;margin-top:18px"><p style="margin:0;font-size:13px;color:#7d6608">💡 <strong>Note:</strong> This is a result notification only. Your marks and lecturer feedback are available securely in the Student Portal.</p></div>
    ...
    async function gradeSubmission(id,release){
        ...
        if(!s)return;
        const assignment=state.assignments.find(a=>a.id===s.assignment_id)||{};
        const gradingConfig=await getAssignmentGradingConfig(assignment);
        const maxMarks=Number(s.max_marks||assignment.max_marks||gradingConfig.markingKey?.max_marks||0);
        if(!maxMarks){notify('This assignment has no maximum mark configured. Open Edit Assignment and set Max Marks before grading.','error');return;}
