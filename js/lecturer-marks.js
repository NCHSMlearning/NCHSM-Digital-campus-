...
// ============================================================
// STANDALONE FUNCTIONS - DEFINED HERE
// ============================================================
...
async function submitMarksForApproval() {
    ...
    if (!existing || existing.length === 0) {
        if (typeof showNotification === 'function') showNotification('No marks to submit for approval', 'warning');
        return;
    }
    ...
    try {
        ...
        if (typeof hideLoading === 'function') hideLoading();
        if (typeof showNotification === 'function') showNotification(`✅ ${existing.length} marks submitted for approval!`, 'success');
        await loadMarksEntry();
        
    } catch (error) {
        if (typeof hideLoading === 'function') hideLoading();
        if (typeof showNotification === 'function') showNotification('❌ Error submitting for approval: ' + error.message, 'error');
    }
}
...
async function withdrawMarksFromApproval() {
    ...
    try {
        ...
        if (typeof hideLoading === 'function') hideLoading();
        if (typeof showNotification === 'function') showNotification('✅ Marks withdrawn from approval!', 'success');
        await loadMarksEntry();
        
    } catch (error) {
        if (typeof hideLoading === 'function') hideLoading();
        if (typeof showNotification === 'function') showNotification('❌ Error withdrawing: ' + error.message, 'error');
    }
}
...
window.detectLecturerProgram = LecturerMarks.detectLecturerProgram.bind(LecturerMarks);
window.showNotification = showNotification;
window.showLoading = showLoading;
window.hideLoading = hideLoading;
...
console.log('✅ loadMESubjects:', typeof loadMESubjects);
