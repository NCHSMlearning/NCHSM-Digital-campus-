...
// SAFE DECLARATIONS - Uses if checks to prevent duplicates
// ============================================================

console.log('🏆 Super Admin Certificate Generator Loading...');

// ============================================================
// SAFE GLOBAL VARIABLES - Only declare if not already defined
// ============================================================

if (typeof certificates === 'undefined') {
    var certificates = [];
    console.log('✅ certificates declared');
}
...
if (typeof initCertificateSystem === 'undefined') {
    window.initCertificateSystem = function() {
...
if (typeof loadCertificates === 'undefined') {
...
if (typeof loadGraduationCandidates === 'undefined') {
    window.loadGraduationCandidates = function() {
        try {
...
if (typeof loadStudentsAndMarks === 'undefined') {
    window.loadStudentsAndMarks = function() {
        try {
        ...
        try {
            const transcripts = JSON.parse(localStorage.getItem(TRANSCRIPT_STORAGE_KEY) || '[]');
            allTranscripts = transcripts;
        } catch (e) {
            allTranscripts = [];
        }
        
        console.log(`📊 Loaded ${allStudents.length} students, ${Object.keys(allMarks).length} with marks`);
    };
}

// ============================================================
// SAFE SETUP EVENT LISTENERS
// ============================================================

if (typeof setupCertificateEventListeners === 'undefined') {
    window.setupCertificateEventListeners = function() {
...
// ============================================================
// SAFE GENERATE UNIQUE SERIAL NUMBER
// ============================================================

if (typeof generateUniqueSerialNumber === 'undefined') {
    window.generateUniqueSerialNumber = function(student, program) {
...
// ============================================================
// SAFE GENERATE QR CODE DATA
// ============================================================

if (typeof generateQRCodeData === 'undefined') {
    window.generateQRCodeData = function(certData) {
        const qrPayload = {
...
// ============================================================
// SAFE CREATE CERTIFICATE
// ============================================================

if (typeof createCertificate === 'undefined') {
    window.createCertificate = function(student, marks, transcriptData) {
        const now = new Date();
...
// ============================================================
// SAFE GET STUDENT MARKS
// ============================================================

if (typeof getStudentMarks === 'undefined') {
    window.getStudentMarks = function(studentId) {
...
// ============================================================
// SAFE GENERATE CERTIFICATES
// ============================================================

if (typeof generateSingleCertificate === 'undefined') {
    window.generateSingleCertificate = function(studentId) {
...
if (typeof generateSelectedCertificates === 'undefined') {
    window.generateSelectedCertificates = function() {
...
if (typeof generateCertificatesForAll === 'undefined') {
    window.generateCertificatesForAll = function() {
...
if (typeof autoGenerateAllCertificates === 'undefined') {
    window.autoGenerateAllCertificates = function() {
...
if (typeof renderGraduateList === 'undefined') {
    window.renderGraduateList = function() {
...
if (typeof renderCertificateList === 'undefined') {
    window.renderCertificateList = function() {
...
if (typeof updateCertStats === 'undefined') {
    window.updateCertStats = function() {
        const totalStudents = allStudents.length;
        const graduates = allStudents.filter(s => s.block === 'Final' || s.block === 'Block 6');
        const issued = certificates.filter(c => c.status === 'ACTIVE').length;
        const pending = graduates.length - issued;
        const scanCount = certificates.reduce((sum, c) => sum + (c.scanCount || 0), 0);
        
        const elements = {
            'certTotalStudents': totalStudents || 0,
            'certGraduatesCount': graduates.length || 0,
            'certIssuedCount': issued || 0,
            'certScanCount': scanCount || 0,
            'certPendingCount': Math.max(pending, 0)
        };
...
// ============================================================
// SAFE QR CODE FUNCTIONS
// ============================================================

if (typeof showCertificateQR === 'undefined') {
    window.showCertificateQR = function(studentId) {
...
if (typeof renderQRCode === 'undefined') {
    window.renderQRCode = function(canvasId, data) {
...
if (typeof verifyCertificate === 'undefined') {
    window.verifyCertificate = function(studentId) {
        const cert = certificates.find(c => c.studentId === studentId);
        if (!cert) {
...
// ============================================================
// SAFE DOWNLOAD CERTIFICATE PDF
// ============================================================

if (typeof downloadCertificatePDF === 'undefined') {
    window.downloadCertificatePDF = function(studentId) {
        const cert = certificates.find(c => c.studentId === studentId);
        if (!cert) {
            if (typeof showNotification === 'function') {
                showNotification('Certificate not found', 'error');
...
if (typeof getProgramFullName === 'undefined') {
    window.getProgramFullName = function(programCode) {
        const programNames = {
...
if (typeof generateCertificateHTML === 'undefined') {
    window.generateCertificateHTML = function(cert) {
...
if (typeof toggleAllCertCheckboxes === 'undefined') {
    window.toggleAllCertCheckboxes = function() {
        const selectAll = document.getElementById('certSelectAll');
...
if (typeof markCertificateAsPrinted === 'undefined') {
    window.markCertificateAsPrinted = function(studentId) {
        const cert = certificates.find(c => c.studentId === studentId);
        if (!cert) return;
...
if (typeof markSelectedAsPrinted === 'undefined') {
    window.markSelectedAsPrinted = function() {
        const checkboxes = document.querySelectorAll('.cert-student-checkbox:checked');
        const selectedIds = Array.from(checkboxes).map(cb => cb.dataset.studentId);
...
if (typeof exportCertificateCSV === 'undefined') {
    window.exportCertificateCSV = function() {
...
if (typeof exportCertificateData === 'undefined') {
    window.exportCertificateData = function() {
...
if (typeof refreshCertificates === 'undefined') {
...
if (typeof filterCertStudents === 'undefined') {
    window.filterCertStudents = function() {
        renderGraduateList();
    };
}
...
if (typeof copySerial === 'undefined') {
...
// ============================================================
// EXPOSE ALL FUNCTIONS TO GLOBAL SCOPE (safely)
// ============================================================

// Only assign if not already defined
if (typeof window.initCertificateSystem === 'undefined') window.initCertificateSystem = initCertificateSystem;
if (typeof window.autoGenerateAllCertificates === 'undefined') window.autoGenerateAllCertificates = autoGenerateAllCertificates;
if (typeof window.generateSingleCertificate === 'undefined') window.generateSingleCertificate = generateSingleCertificate;
if (typeof window.generateSelectedCertificates === 'undefined') window.generateSelectedCertificates = generateSelectedCertificates;
if (typeof window.generateCertificatesForAll === 'undefined') window.generateCertificatesForAll = generateCertificatesForAll;
if (typeof window.showCertificateQR === 'undefined') window.showCertificateQR = showCertificateQR;
if (typeof window.verifyCertificate === 'undefined') window.verifyCertificate = verifyCertificate;
if (typeof window.downloadCertificatePDF === 'undefined') window.downloadCertificatePDF = downloadCertificatePDF;
if (typeof window.downloadQR === 'undefined') window.downloadQR = downloadQR;
if (typeof window.markCertificateAsPrinted === 'undefined') window.markCertificateAsPrinted = markCertificateAsPrinted;
if (typeof window.markSelectedAsPrinted === 'undefined') window.markSelectedAsPrinted = markSelectedAsPrinted;
if (typeof window.exportCertificateCSV === 'undefined') window.exportCertificateCSV = exportCertificateCSV;
if (typeof window.exportCertificateData === 'undefined') window.exportCertificateData = exportCertificateData;
if (typeof window.refreshCertificates === 'undefined') window.refreshCertificates = refreshCertificates;
if (typeof window.filterCertStudents === 'undefined') window.filterCertStudents = filterCertStudents;
if (typeof window.toggleAllCertCheckboxes === 'undefined') window.toggleAllCertCheckboxes = toggleAllCertCheckboxes;
if (typeof window.copySerial === 'undefined') window.copySerial = copySerial;
if (typeof window.renderCertificateList === 'undefined') window.renderCertificateList = renderCertificateList;
if (typeof window.renderGraduateList === 'undefined') window.renderGraduateList = renderGraduateList;
if (typeof window.updateCertStats === 'undefined') window.updateCertStats = updateCertStats;
if (typeof window.getProgramFullName === 'undefined') window.getProgramFullName = getProgramFullName;
if (typeof window.isNursingProgram === 'undefined') window.isNursingProgram = isNursingProgram;
if (typeof window.isTVETProgram === 'undefined') window.isTVETProgram = isTVETProgram;
if (typeof window.getGradingConfig === 'undefined') window.getGradingConfig = getGradingConfig;
if (typeof window.calculateOfficialGrade === 'undefined') window.calculateOfficialGrade = calculateOfficialGrade;
if (typeof window.escapeHtml === 'undefined') window.escapeHtml = escapeHtml;
if (typeof window.generateQRCodeData === 'undefined') window.generateQRCodeData = generateQRCodeData;
if (typeof window.generateUniqueSerialNumber === 'undefined') window.generateUniqueSerialNumber = generateUniqueSerialNumber;
if (typeof window.createCertificate === 'undefined') window.createCertificate = createCertificate;
if (typeof window.getStudentMarks === 'undefined') window.getStudentMarks = getStudentMarks;
if (typeof window.loadCertificates === 'undefined') window.loadCertificates = loadCertificates;
if (typeof window.loadGraduationCandidates === 'undefined') window.loadGraduationCandidates = loadGraduationCandidates;
if (typeof window.loadStudentsAndMarks === 'undefined') window.loadStudentsAndMarks = loadStudentsAndMarks;
if (typeof window.setupCertificateEventListeners === 'undefined') window.setupCertificateEventListeners = setupCertificateEventListeners;

console.log('🏆 Certificate System Loaded Successfully! (with if checks)');
...
console.log('📋 Run initCertificateSystem() to initialize');
console.log('📋 Or run generateTestCertificate() to create a test certificate (if available)');
