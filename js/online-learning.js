...
async function saveInlineResearchCorrection(){
  ...
  if(!base||!editor){alert('The correction editor is not ready. Please reopen the paper and try again.');return;}
  if(!db||!id){alert('Your student session is not available. Please sign in again and reload the portal.');return;}
...
function bindResearch(){
  ...
  if(modal&&!modal.dataset.researchViewerControls){
    ...
    modal.addEventListener('click',function(e){
      ...
      var submitCorrection=e.target.closest('[data-rs-submit-correction]');
      if(submitCorrection){e.preventDefault();submitCorrection.disabled=true;Promise.resolve(saveInlineResearchCorrection()).finally(function(){submitCorrection.disabled=false});return}
      var viewOnly=e.target.closest('[data-rs-view-only]');
      if(viewOnly){e.preventDefault();if(state.researchCurrent)loadResearchDocumentForViewer(state.researchCurrent,false);return}
      var closePaper=e.target.closest('[data-research-close]');
      if(closePaper){e.preventDefault();closeResearchModal();return}
    });
  }
