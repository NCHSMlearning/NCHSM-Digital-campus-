...
class ResourcesModule {
    ...
    showAudioPlayer(text, title) {
        ...
        if (this.audioWordCount) {
            this.audioWordCount.textContent = text ? (wordCount + ' words') : 'Pre-recorded audio';
        }
        ...
        if (this.audioDuration) {
            if (text) {
                ...
                this.audioDuration.textContent = '0:00';
            }
        }
    }
    ...
    async openResource(resourceId) {
        ...
        if (fileType === 'pdf') {
            ...
            await this.openDocumentInPortal(resource);
        }
    }
    
    async openDocumentInPortal(resource) {
        const existing = document.getElementById('resource-document-reader');
        if (existing) existing.remove();

        const modal = document.createElement('div');
        modal.id = 'resource-document-reader';
        modal.style.cssText = 'position:fixed;inset:0;background:rgba(15,23,42,.96);z-index:100001;display:flex;align-items:center;justify-content:center;padding:12px;';

        const safeTitle = this.escapeHtml(resource.title || 'Study Material');
        const url = resource.file_url || '';

        modal.innerHTML = `
            <div style="width:100%;height:100%;max-width:1200px;background:#fff;border-radius:16px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,.5);">
                <div style="height:58px;flex-shrink:0;background:linear-gradient(135deg,#4c1d95,#7c3aed);color:#fff;display:flex;align-items:center;justify-content:space-between;padding:0 16px;">
                    <strong style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">📖 ${safeTitle}</strong>
                    <button id="resource-document-close" style="border:0;background:rgba(255,255,255,.15);color:#fff;width:38px;height:38px;border-radius:8px;cursor:pointer;"><i class="fas fa-times"></i></button>
                </div>
                <div style="flex:1;position:relative;background:#f1f5f9;">
                    <iframe src="${url}" title="${safeTitle}" style="width:100%;height:100%;border:0;" sandbox="allow-same-origin allow-scripts"></iframe>
                    <div style="position:absolute;left:12px;bottom:12px;background:rgba(15,23,42,.86);color:#fff;padding:7px 12px;border-radius:20px;font-size:12px;"><i class="fas fa-lock"></i> Read Only · In Portal</div>
                </div>
            </div>`;
        document.body.appendChild(modal);

        const close = () => { modal.remove(); document.body.style.overflow = ''; };
        modal.querySelector('#resource-document-close')?.addEventListener('click', close);
