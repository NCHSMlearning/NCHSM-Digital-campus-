...
class Database {
    ...
    async loadConfiguration() {
        try {
            ...
            if (window.APP_CONFIG) {
                console.log('✅ Configuration already loaded from config.js');
                return true;
            }
            
            // For GitHub Pages deployment (config.js generated by GitHub Actions)
            console.log('🔄 Loading configuration...');
            
            // Try to fetch config.js
            const response = await fetch('./config.js');
            if (!response.ok) {
                throw new Error('config.js not found');
            }
            
            // The config.js file will set window.APP_CONFIG
            // It's loaded as a script in HTML, so we just need to wait for it
            ...
            console.warn('⚠️ Could not load external config, checking for local config...');
    ...
    getLocalConfig() {
        ...
        if (isLocal) {
            ...
            // Try to get from localStorage first (for testing different credentials)
            ...
            return {
                ...
                SUPABASE_ANON_KEY: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBsYWNlaG9sZGVyIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTAwMDAwMDAsImV4cCI6MjAwMDAwMDAwMH0.placeholder",
                LOCATIONIQ_API_KEY: "pk_local_dev_key",
                BUILD_TIME: "local-dev-" + new Date().toISOString(),
    ...
    async initialize() {
        ...
        try {
            ...
            console.log('🔧 Using Supabase URL:', window.APP_CONFIG.SUPABASE_URL.substring(0, 30) + '...');
    ...
    async testConnection() {
        try {
            ...
            if (error) {
                // If table doesn't exist, try a different test
                const { error: authError } = await this.supabase.auth.getSession();
                if (authError) {
                    throw new Error('Supabase authentication failed: ' + authError.message);
                }
            }
            ...
            ">
                ...
                ">
                    <div style="margin-bottom: 30px;">
                        ...
                        <h1 style="color: #1f2937; margin-bottom: 10px;">Database Connection Error</h1>
                    ...
                    ">
                        ...
                        <ol style="color: #374151; margin-left: 20px;">
                            <li>Check GitHub Secrets for SUPABASE_URL and SUPABASE_ANON_KEY</li>
                            <li>Ensure Supabase project is running and accessible</li>
                    ...
                    <div style="display: flex; gap: 10px; justify-content: center;">
                        ...
                        <button onclick="showConfigHelp()" style="
                            background: #6b7280;
                            color: white;
                            border: none;
                            padding: 12px 24px;
                        ...
                        <button onclick="localStorage.setItem('dev_config', JSON.stringify(prompt('Enter dev config JSON:'))) && location.reload()" style="
                            background: #f59e0b;
                            color: white;
                            border: none;
                            padding: 12px 24px;
                            ...
                            gap: 8px;
                        ">
                            <i class="fas fa-tools"></i> Dev Mode
                        </button>
                    </div>
...
// Helper function for configuration help
function showConfigHelp() {
    const helpText = `
# Configuration Help

## For Local Development:
1. Create a file called 'config.js' in your project root
...
1. Is window.APP_CONFIG defined?
2. Are SUPABASE_URL and SUPABASE_ANON_KEY present?
3. Check network tab for config.js loading
    `;
